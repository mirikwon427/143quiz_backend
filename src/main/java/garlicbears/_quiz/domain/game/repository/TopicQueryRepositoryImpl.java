...
public class TopicQueryRepositoryImpl implements TopicQueryRepository {
	...
	public Page<ResponseTopicDto> findTopics(int page, int size, String sortBy, Pageable pageable) {
		...
		List<ResponseTopicDto> results = queryFactory.select(
				Projections.constructor(ResponseTopicDto.class, topic.topicId, topic.topicTitle, topic.topicActive,
					topic.createdAt, topic.updatedAt, topic.topicUsageCount))
			.from(topic)
		...
		Long total = queryFactory.select(topic.count()).from(topic).fetchOne();
